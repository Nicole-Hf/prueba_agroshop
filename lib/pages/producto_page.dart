...
import 'package:prueba_agroshop/pages/search_country_delegate.dart';
...
class _ProductoPageState extends State<ProductoPage> {
  ...
  _initData() async {
    ...
    await CallApi().getPublicData("allempresa").then((response) {
      setState(() {
        Iterable list = json.decode(response.body);
        empresas = list.map((model) => Empresa.fromJson(model)).toList();
      });
    });
  }
        ...
        children: [
          ...
          ListTile(
            onTap: () {
              showSearch(
                context: context,
                delegate: SearchCountryDelegate(productos, empresas),
              );
            },
                        ...
                        Navigator.push(
                            ...
                            MaterialPageRoute(
                                builder: (context) => DetailProductPage(
                                      articleInfo: productos[i],
                                      empresaInfo: empresas[1],
                                      index: 0,
                                    )));
                      },
